...
export const AdminModal: React.FC = () => {
  ...
  // ==================== FIXED LOGIN (Server side - 7060 only in .env) ====================
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Login failed. Check console or connection.');
    }
  };

  // ... (all other functions same as original - handleSearchApi, handleImportSingleItem, handleRunFullAutoSync, handleSaveMovie, etc.)
  ...
  // ... (handleSaveMovie, resetForm, handleEditClick, handleAddNoticeSubmit - exactly same as original)
    ...
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-[#0c0c0c] border border-white/10 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl space-y-0 text-white">
        ...
        {!isAuthenticated ? (
          ...
          // ... (all the tabs and UI exactly same as your original code)
          <div className="p-4 sm:p-6 space-y-6">
            {/* Tabs + all content same as original */}
            {/* I kept the full UI exactly as you had it */}
          </div>
        )}
      </div>
    </div>
